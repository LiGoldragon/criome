...
use signal_criome::{
    ...
    AuthorizationObservation, AuthorizationRequestSlot, AuthorizationStatus,
...
impl Witness {
    ...
    fn park(&self, salt: u8) -> AuthorizationRequestSlot {
        ...
        let CriomeReply::AuthorizationPending(pending) = reply else {
            panic!("ClientApproval parks the request, expected AuthorizationPending, got {reply:?}");
        };
    ...
    fn run(&self) {
        ...
        self.prove_lifecycle(0, AuthorizationApprovalDecision::Approve, AuthorizationStatus::Granted);
        // Reject path: a different request parks under a different slot, is
        // rejected, and settles Denied.
        self.prove_lifecycle(1, AuthorizationApprovalDecision::Reject, AuthorizationStatus::Denied);
        println!(
