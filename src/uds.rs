...
//! and shuttles them through a shared [`Daemon`] handle.
//!
...
impl Listener {
    ...
    /// independently in its own tokio task and shares the
    /// supplied `Daemon` via `Arc`.
    pub async fn run(self, daemon: Arc<Daemon>) -> Result<()> {
        loop {
            let (socket, _) = self.listener.accept().await?;
            let daemon = Arc::clone(&daemon);
            tokio::spawn(async move {
                if let Err(error) = Self::handle_connection(socket, daemon).await {
    ...
    async fn handle_connection(mut socket: UnixStream, daemon: Arc<Daemon>) -> Result<()> {
        loop {
            ...
            Self::write_frame(&mut socket, reply_frame).await?;
        }
        Ok(())
    }
