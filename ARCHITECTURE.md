...
record kind is a closed Rust type generated by rsc from a
KindDecl record in sema.

The engine speaks in narrow, named types — `AssertOp::Node`,
`MutateOp::Edge { slot, new, expected_rev }`, `Records::Graph
(Vec<Graph>)` — never `Request(GenericRecord)` or
`Records(Vec<AnyKind>)`. A query is its own kind paired with
the instance kind it queries; an `Unknown` escape hatch does
not exist; reply payloads are typed per query. The schema is
data (`KindDecl` records); the typed code is its projection.
...
- **Closed enums at the wire.** Adding a kind = asserting a
  KindDecl + recompiling (the §7 self-host loop). No string
  kind-name lookup at runtime.
- **Per-verb payload types.** AssertOp ≠ MutateOp ≠ QueryOp;
  each is its own enum because each carries a different
  shape per kind.
...
- **Schema-as-data + typed-as-projection.** KindDecl records
  in sema are canonical; the closed Rust enum is rsc's
  projection.
