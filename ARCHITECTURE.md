...
does not exist; reply payloads are typed per query. The
schema is data (`KindDecl` records); the typed code is its
projection.
...
- **Layer 1 — schema vocabulary**: lives inside
  ...
  declarations as `KindDecl` + `FieldDecl` + `Cardinality`
  types; plus the data-kind structs `Node` / `Edge` / `Graph`
  and their paired `*Query` types; plus the IR types —
  `AssertOperation` / `MutateOperation` / `RetractOperation` /
  `QueryOperation` / `BatchOperation` / `AtomicBatch` /
  `Records`, `Diagnostic`). The former separate
  `nexus-schema` crate was absorbed into signal.
