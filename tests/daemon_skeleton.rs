...
#[test]
fn authorization_observation_stream_pushes_approval_update() {
    let workspace = fixture_path("authorization-observation-push");
    let socket = workspace.join("criome.sock");
    let meta_socket = workspace.join("criome-meta.sock");
    let store = StoreLocation::new(workspace.join("criome.sema"));
    let daemon = CriomeDaemon::new(&socket, store.clone())
    ...
    thread::scope(|scope| {
        let server = scope.spawn(|| {
            daemon
                ...
                .expect("serve authorization stream")
        });
        let mut session = CriomeClient::new(&socket)
            .observe_authorization(signal_criome::AuthorizationObservation::new(
                pending.request_slot.clone(),
            ))
            .expect("open authorization observation");
        session
            .set_read_timeout(Some(std::time::Duration::from_secs(2)))
            .expect("set stream timeout");
        assert_eq!(session.token().payload(), &pending.request_slot);
        ...
        assert_eq!(snapshot_states[0].request_slot, pending.request_slot);
        ...
        assert_eq!(update.request_slot, pending.request_slot);
        ...
        assert_eq!(server.join().expect("join authorization stream"), None);
    });
